...
namespace DFSNetworking
{
		...
		while (1)
		{
			...
			{ 	
				...
				else
				{
					...
					{
						NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_SYSTEM, "Network Thread[" + std::to_string(NetworkThreadID) + "] has been idle for 2 minutes and is self destructing.");
						delete this;
						return;
					}
				}
			...
			while (NetworkThreadMessenger && NetworkThreadMessenger->HasMessages())
			{
				...
				{
					NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_ALL, "Shutdown signal received, shutting down thread.");
					delete this;
					return;
				}
				...
				if (NewMessage.Pointer && NewMessage.acceptTask())
				{
					...
					NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_CONNECTIONS, "New connection accepted from " + std::string(NewConnection->GetIP()));
				}				
			}
			...
			for (int ConnectionListIterator = 0; ConnectionListIterator < ConnectionList.size(); ConnectionListIterator++)
			{
					...
					PollStruct[ConnectionListIterator].revents & POLLOUT)
				{
						...
						&& ConnectionList[ConnectionListIterator]->SendBuffer.empty())
					{
						...
						NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_FILEACCESS, std::string(ConnectionList[ConnectionListIterator]->GetIP()) + " - [" + ConnectionList[ConnectionListIterator]->Resource + "]");
						NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_FILEACCESS, "Resolved resource to: -> " + std::string(Resource));
								...
								{ // Try to verify the authentication.
									...
									if (Base64Encoding.decode(Base64Authorization) != Configuration.BasicCredentials)
									{
										if (Base64Authorization == "")
											NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_FILEACCESS, std::string(ConnectionList[ConnectionListIterator]->GetIP()) + " - Protected Resource Requested; Authentication Requested.");
										else
											NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_FILEACCESS, std::string(ConnectionList[ConnectionListIterator]->GetIP()) + " - Protected Resource Requested; Failed Authentication.");
	...
	NetworkThread::NetworkThread(bool aprimeThread)
	{
		...
		if (MessengerServer)
		{
			...
			NetworkThreadMessenger->sendMessage(MSG_TARGET_CONSOLE, VISIBILITY_SYSTEM, "Network Thread initialized.");
		}
		else
			NetworkThreadMessenger = nullptr;
